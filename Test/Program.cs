using Kodlama.ioSimulation.DataAccess.Concretes.InMemory;
...
{
    Console.WriteLine($"{course.CategoryId}  {course.InstructorId}   {course.Name}       {course.Description}");
}
...
courseManager.Add(new CreateCourseRequest
{
    CategoryId = 1,
    InstructorId = 1,
    Name = "Swift Yazılımcı Yetiştirme Kampı",
...
static void GetCourseList(CourseManager courseManager)
{
    ...
    {
        Console.WriteLine($"Kategori id={course.CategoryId} ve Instructor id= {course.InstructorId} olan kursun adı: {course.Name} \n Açıklama: {course.Description} \n Image Url: {course.ImageUrl} \n Price: {course.Price}");
        Console.WriteLine("------------------------------------------------------------------------");
    }
}